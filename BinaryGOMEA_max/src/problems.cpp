...
bool problemNameByIndex(Config *config, string &problemName)
{
	switch (config->problemIndex)
	{
		...
		case 1: problemName = "Concatenated Deceptive Trap with k=" + to_string(4) + " s=" + to_string(3); break;
...
double deceptiveTrap(int unitation, int k)
{
	double result = 0.0;
	if (unitation != k)
		result = 0.8 * (double) (k - 1 - unitation)/(k - 1);
	else
		result = (double) unitation/unitation;
...
double concatenatedDeceptiveTrap::calculateFitness(Individual *solution)
{
	...
	// 		int _temp = solution->genotype[j%numberOfVariables];
	// 		// cout << _temp << " ";
