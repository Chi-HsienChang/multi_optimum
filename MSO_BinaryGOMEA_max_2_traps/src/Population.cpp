  ...
  *offspringPopulation[offspringIndex] = *population[offspringIndex];
  ...
  for (size_t i = 0; i < FOSInstance->FOSSize(); i++)
  {
    ...
    while (donorEqualToOffspring && indicesTried < donorIndices.size())
    {
      ...
      // cout << numberOfGenerations <<","<< FOSInstance->FOSElementSize(ind) << endl; // Salima All FOS
      int changedgenes = 0;
      ...
      if (!donorEqualToOffspring)
      {
        ...
        {     
          ...
          // cout << offspringPopulation[offspringIndex]->fitness << endl;
          // cout << numberOfGenerations <<","<< FOSInstance->FOSElementSize(ind) << endl; // Salima Success FOS  
