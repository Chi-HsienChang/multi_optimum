  ...
  for(i = 0; i < IMSsubgenerationFactor-1; i++)
  {
    for(GOMEAIndex = GOMEAIndexSmallest; GOMEAIndex <= GOMEAIndexBiggest; GOMEAIndex++)
    {
      ...
      if((!GOMEAs[GOMEAIndex]->terminated) && (GOMEAIndex >= minimumGOMEAIndex))
      {
        ...
        // cout <<  GOMEAIndex << " " << GOMEAs[GOMEAIndex]->numberOfGenerations << " " << sharedInformationInstance->numberOfEvaluations << " " << GOMEAs[GOMEAIndex]->population.size() << " " << GOMEAs[GOMEAIndex]->averageFitness << "!" << endl; // Salima
