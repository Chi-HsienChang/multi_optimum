...
void writeElitistSolutionToFile(string &folder, long long numberOfEvaluations, long long time, Individual *solution)
{
	...
	outFile << (int)numberOfEvaluations << " " << time << " " <<  fixed << setprecision(2) << solution->fitness << " ";
	for (size_t i = 0; i < solution->genotype.size(); ++i)
	{	if (solution->fitness >= 100) // Salima
			cout << +solution->genotype[i];
		outFile << +solution->genotype[i];
	}

	// if (solution->fitness >= 100) // Salima
	// {
	// 	cout << " " <<  fixed << setprecision(2) << solution->fitness << " Evaluations " << (int)numberOfEvaluations;
	// 	cout << endl;
	// }
